...
public class Buyer {
	...
	private int mango; // 소지망고갯수
	private int kiwi; // 소지키위갯수
	
	// 생성자에 돈을 받아서 소지금액을 초기화해주고 망고는 0으로 초기화
	public Buyer(int money) {
		this.money = money;
		this.mango = 0;
		this.kiwi = 0;
	}
	// buyMango(Seller seller, int mango)를 정의하는데 어떤 상인에게, 몇 개를 살지
	public void buyMango(Seller seller,int mango) {
		...
		// 판매자의 망고가 내가 구매하려는 망고보다 적은경우도 역시 "망고재료가 부족합니다."라고만 알리고 메서드를 중지시킵니다.
	    ...
	    // 내 금액을 차감되고 망고가 늘어나는 로직
	    this.money -= mango*4000;
	    this.mango += mango;
	    // 이때 seller쪽에서도 sellMango()가 호출되도록 처리
	...
	public void buykiwi(Seller seller,int kiwi) {
		...
		if(seller.getkiwi() < kiwi) {
			System.out.println("키위재고가 부족합니다. 망고 재고 : " + seller.getkiwi());
			return;
		}
	...
	// showBuyer()을 이용해 소지 망고, 소지금을 볼 수 있습니다.
	public void showBuyer() {
		System.out.println("----------구매자 정보------------");
		System.out.println("소지중인 망고개수 :" + mango + ", 소지중인 키위개수 : " + kiwi + ", 잔여금액 : " + money);
		System.out.println("-------------------------------");
	}
